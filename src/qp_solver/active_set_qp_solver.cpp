...
namespace control{
    ...
    bool ActiveSetQpSolver::Solve() {
        ...
        if (kernel_matrix_.rows() != kernel_matrix_.cols()) {
            std::cout << "kernel_matrix_.rows() [" << kernel_matrix_.rows()
        ...
        // DCHECK_EQ(index, affine_equality_matrix_.rows() * num_param_ + affine_inequality_boundary_.rows() * num_param_);
        ...
        if (ret != qpOASES::SUCCESSFUL_RETURN) {
            if (ret == qpOASES::RET_MAX_NWSR_REACHED) {
                ...
                std::cout << "qpOASES solver failed due to infeasibility or other internal "
                          "reasons";
            }
            std::stringstream ss;
